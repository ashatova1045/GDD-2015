...
{
    public partial class horarioInicial : Form
    {
        private TimeSpan horarioInic = new TimeSpan(0);  // Backing store
        ...
        public horarioInicial(string dia)
        {
            InitializeComponent();
            tBoxDia.Text = dia;
        ...
        {
            TimeSpan horarioInicial = TimeSpan.FromHours(7);
            ...
            if (TimeSpan.TryParse(horario, out span))
            {
                ...
                {
                    if (span.CompareTo(horarioInicial) < 0)
                    {
                        // HORARIO ANTERIOR AL INICIAL
